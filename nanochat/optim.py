    ...
    def _reduce_adamw(self, group: dict, world_size: int) -> dict:
        ...
        for p in group['params']:
            ...
            if p.numel() < 1024 or grad.shape[0] % world_size != 0:
                # Small params or params with shape[0] not divisible by world_size:
                # all_reduce (no scatter/gather needed, state replicated across ranks)
                future = dist.all_reduce(grad, op=dist.ReduceOp.AVG, async_op=True).get_future()
                param_infos[p] = dict(future=future, grad_slice=grad, is_small=True)
            else:
                # Large params: reduce_scatter (shape[0] is guaranteed divisible by world_size)
